...
def feature_selection_rf(data):
    ...
    # data["hdcat"] = labelencoder_hdcat.fit_transform(data["hdcat"]) #Fitting object to the 2nd
    ...
    # data = onehotencoder.fit_transform(data).toarray() #Putting the variable before the =
    #     # again transforms it, this time using the onehotencoder object we created
    # data = data[:, 1:] # Removing one dummy coded column to avoid falling into "Dummy
    ...
    # data_columns = list(data) #fancy imputation function removes column names, so we retain them and reassign them to the imputed data
    # # print("data_columns", data_columns)
    # print("len(data_columns)", len(data_columns))
    # imp = SimpleImputer(strategy='mean', missing_values=np.nan or "NaN")  #for mode we specify strategy='most_frequent'
    # imp.fit(data) # PROBLEM - not imputing all NaNs
