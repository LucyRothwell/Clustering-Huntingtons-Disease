...
class ZscoreSustain(AbstractSustain):
    ...
    def __init__(self,
        ...
        # Parameters:
        #   data                        - !important! needs to be (positively increasing towards 0) z-scores! Need to look at each Z mean compared to control. If all case means are positive it's fine - become more positive over time. If all have negative that's fine. If some are positively increasing an some are negatively increasing there's a prob. If case mean is lower (neg) than case mean, * it by -1
        #                                 Check mean of all predictors and whether lower/higher than control means. Depends score - some get higher or lower as diease progresses. Ex brain tissue decreases but fluid goes up. You need to flip flui so it's going down also.
    ...
    def _optimise_parameters(self, sustainData, S_init, f_init):
        ...
        for s in range(N_S):
            ...
            # print("p_perm_k =", p_perm_k) # Full of numbers lucy*
        p_perm_k_weighted                   = p_perm_k * f_val_mat
        # print("p_perm_k_weighted =", p_perm_k_weighted)   # Full of numbers              #lucy***
        # WARNING LINE v - Trying to divide by 0?                                                 lucy***
        ...
        # print("p_perm_k_norm", p_perm_k_norm) # FULL OF NUMBERS lucy***
        # print("np.tile(np.sum(np.sum(p_perm_k_weighted, 1)", np.tile(np.sum(np.sum(p_perm_k_weighted, 1), 1)))  #lucy***
        f_opt                               = (np.squeeze(sum(sum(p_perm_k_norm))) / sum(sum(sum(p_perm_k_norm)))).reshape(N_S, 1, 1)
        # print("f_val_mat 247", f_val_mat)
        # print("f_opt =", f_opt)
        # print("N =", N)
        # print("M =", M)
        f_val_mat                           = np.tile(f_opt, (1, N + 1, M))
        # print("f_val_mat 249", f_val_mat)
        f_val_mat                           = np.transpose(f_val_mat, (2, 1, 0))
        # print("f_val_mat 251", f_val_mat)  # *lucy
        ...
        for s in order_seq:
            ...
            for i in order_bio:
                ...
                for index in range(len(possible_positions)):
                    ...
                    # print("p_perm_k [:, :, s]", p_perm_k [:, :, s]) # >> FULL OF NUMBERS        lucy*
                    # print("f_val_mat 305", f_val_mat) # >> A NAN VALUE    lucy*
                    total_prob_stage        = np.sum(p_perm_k * f_val_mat, 2) # PROBEM HERE >> f_val_mat is NANs***
                    # print("total_prob_stage =", total_prob_stage)  # lucy*** added >>> ALL NANS
                    total_prob_subj         = np.sum(total_prob_stage, 1)
                    # print("total_prob_subj =", total_prob_subj)  # lucy*** added >>> ALL NANS
                    possible_likelihood [index] = sum(np.log(total_prob_subj + 1e-250))
                    # print("possible_likelihood [index] =", possible_likelihood [index])  # lucy*** added >>> ALL NANS
                possible_likelihood         = possible_likelihood.reshape(possible_likelihood.shape [0])
                # print("possible_likelihood =", possible_likelihood) # >> NAN         lucy*** added
                max_likelihood              = max(possible_likelihood)
                # print("max_likelihood =", max_likelihood) #  >> NAN      lucy*** added
        ...
        p_perm_k_norm                       = p_perm_k_weighted / np.tile(np.sum(np.sum(p_perm_k_weighted, 1), 1).reshape(M, 1, 1), (1, N + 1, N_S))  # the second summation axis is different to Matlab version
        f_opt                               = (np.squeeze(sum(sum(p_perm_k_norm))) / sum(sum(sum(p_perm_k_norm)))).reshape(N_S, 1, 1)
        # print("f_opt 327", f_opt) # *lucy
        f_val_mat                           = np.tile(f_opt, (1, N + 1, M))
        f_val_mat                           = np.transpose(f_val_mat, (2, 1, 0))
        # print("f_val_mat 330", f_val_mat)  # *lucy
